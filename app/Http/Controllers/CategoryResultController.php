...
class CategoryResultController extends Controller
{
    public function index(Request $request) {
        ...
        foreach ($grouped as $key => $items) {
            ...
            // Per-judge: sum parent weighted_score (already rolled up from FE)
            ...
            if ($debugContestantId && $debugCategoryId
                ...
                $debug[] = [
                    ...
                    'per_judge_totals' => $byJudgeParentSum, // will show 92.693, 93.746, ...
            ...
            // Apply category weight, then average across ALL assigned judges
            $categoryWeight = ((float) ($category->weight ?? 0)) / 100.0;

            $sumAcrossJudgesAfterWeight = $byJudgeParentSum->sum() * $categoryWeight;
            $finalCategoryAverage = $totalJudgesInPhase > 0
                ? $sumAcrossJudgesAfterWeight / $totalJudgesInPhase
                : 0.0;

            // Also expose avg pre-category (helpful for cross-checking)
            ...
            $results[] = [
                ...
                'average_score'     => round($avgPreCategory, 5),     // e.g., should be 92.01070
        ...
        DB::transaction(function () use ($ranked) {
            foreach ($ranked as $r) {
                \App\Models\CategoryResult::updateOrCreate(
                    ['event_id' => $r['event_id'], 'category_id' => $r['category_id'], 'contestant_id' => $r['contestant_id']],
                    [
                        'phase_id'       => $r['phase_id'],
